...
def send_image(sender_id,image_path):
    bot.send_text_message(sender_id, image_path)
...
def send_response(sender_id, intent, value):
    ...
    # TODO: Light:
        ...
        if light.is_light_on():
            ...
            result = 'no'
        response = get_sensor_reply('light', intent, result)
    ...
    # Camera:
        ...
        response = get_sensor_reply('camera','any','hold_on')
        bot.send_text_message(sender_id, response)
        response = None
        time.sleep(2) # making a gap between "hold_on" response and send image
        image_path = camera.capture_image()
        send_image(sender_id, image_path)

    elif intent == 'get_images':
        response = get_sensor_reply('camera','any','hold_on')
        bot.send_text_message(sender_id, response)
        response = None
        time.sleep(2) # making a gap between "hold_on" response and send images
        if value:
            for x in value:
                image_path = camera.capture_image()
                send_image(sender_id, image_path)
